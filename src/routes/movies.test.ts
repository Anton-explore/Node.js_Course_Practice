...
describe('[Movies] API', () => {
  ...
  describe('POST /', () => {
    const movie = { title: 'testMovie', description: 'joking', releaseDate: '12-12-2023', genre: ['comedy'] };
    ...
    it('should return status 404 if movie already exist', async () => {
      ...
      expect(response.body.message).toBe('Movie is already exist!');
    });
  ...
  describe('DELETE /:id', () => {
    ...
    it('should handle the case when the movie to delete is not found', async () => {
      ...
      expect(response.status).toBe(404);
      expect(response.body).toEqual({ message: 'cannot find movie with ID 123' });
    });
